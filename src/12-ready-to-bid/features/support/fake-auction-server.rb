...
class FakeAuctionServer
  ...
  def has_received_join_request_from?(sniper_id)
    @message_listener.with_next_message do | message | 
      assert { message.body == Message.join_message } 
      assert { sniper_id == @current_chat.participant }
    end
  end

  def report_price(price, increment, bidder)
    @current_chat.send_message(Message.price_message(price, increment, bidder))
  end

  def has_received_bid?(bid, sniper_id)
    @message_listener.with_next_message do | message |
      assert { message.body == Message.bid_message("Price", bid) }
      assert { sniper_id == @current_chat.participant }
    end
  end

  def announce_closed
    @current_chat.send_message(Message.empty_message)
  end
